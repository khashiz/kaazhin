...
$input = '<input type="search" name="q" id="mod-finder-searchword' . $module->id . '" class="uk-input uk-width-1-1 js-finder-search-query form-control" value="' . htmlspecialchars($app->input->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
...
if ($params->get('show_button', 0))
{
	...
	$output .= $input;
	$output .= '<button class="btn btn-primary" type="submit">' . Text::_('JSEARCH_FILTER_SUBMIT') . '</button>';
	$output .= '</div>';
}
else
{
...
?>
<form class="uk-margin-medium-bottom uk-position-relative mod-finder js-finder-searchform form-search" action="<?php echo Route::_($route); ?>" method="get" role="search">
    <i class="far fa-search uk-position-center-left uk-text-muted uk-position-z-index"></i>
	<?php echo $output; ?>
